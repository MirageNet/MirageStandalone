...
namespace Mirage
{
    public static class GameobjectExtension
    {
        ...
        {
            //TODO: if (!gameObject.TryGetComponent(out NetworkIdentity identity))
            //{
            //    throw new InvalidOperationException($"Gameobject {gameObject.name} doesn't have NetworkIdentity.");
            //}
            return new NetworkIdentity();// identity;
        }
    }
    ...
    public class ServerObjectManager : MonoBehaviour//, IServerObjectManager
    {
        ...
        public void Start()
        {
            if (Server != null)
            {
                ...
                Server.OnStartHost += StartedHost;
                Server.Stopped += OnServerStopped;

                if (NetworkSceneManager != null)
                {
                    NetworkSceneManager.OnServerFinishedSceneChange += OnFinishedSceneChange;
                    NetworkSceneManager.OnPlayerSceneReady += SpawnVisibleObjects;
                }
            }
        }
        ...
        void StartedHost()
        {
            //TODO: if (TryGetComponent(out ClientObjectManager ClientObjectManager))
            //{
        ...
        public void SpawnObjects()
        {
            ...
            NetworkIdentity[] identities = new NetworkIdentity[1];//TODO: Resources.FindObjectsOfTypeAll<NetworkIdentity>();
