...
namespace Mirage
{
    public class ClientObjectManager : MonoBehaviour, IClientObjectManager
    {
        ...
        public void Start()
        {
            if (Client != null)
            {
                ...
                if (NetworkSceneManager != null)
                    NetworkSceneManager.OnClientFinishedSceneChange += OnFinishedSceneChange;
            }
        }
        ...
        NetworkIdentity SpawnPrefab(SpawnMessage msg)
        {
            ...
            if (!(prefab is null))
            {
                NetworkIdentity obj = Instantiate(prefab, msg.position, msg.rotation) as NetworkIdentity;
        ...
        {
            if (identity != null && identity == Client.Player?.Identity)
            {
        ...
        internal (Task<T> task, int replyId) CreateReplyTask<T>()
        {
            int newReplyId = replyId++;
            var completionSource = new TaskCompletionSource<T>();//AutoResetUniTaskCompletionSource<T>.Create();
