...
namespace Mirage
{
    ...
    public abstract class NetworkBehaviour : MonoBehaviour
    {
        ...
        protected internal Task<T> SendServerRpcWithReturn<T>(Type invokeClass, string cmdName, NetworkWriter writer, int channelId, bool requireAuthority = true)
        {
            ValidateServerRpc(invokeClass, cmdName, requireAuthority);

            (Task<T> task, int id) = ClientObjectManager.CreateReplyTask<T>();
        ...
        {
            lastSyncTime = DateTime.UtcNow.Ticks;// Time.time;
            SyncVarDirtyBits = 0L;
        ...
        public bool IsDirty()
        {
            if (DateTime.UtcNow.Ticks - lastSyncTime >= syncInterval)
            {
